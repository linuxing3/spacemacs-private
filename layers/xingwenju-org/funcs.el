...
;;
;; Author: xingwenju <guanghui8827@gmail.com>
...
(defun xingwenju/insert-semicolon-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (insert ";")))

(defun xingwenju/delete-semicolon-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (if (looking-back ";")
...
(defun xingwenju/insert-comma-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (insert ",")))

(defun xingwenju/delete-comma-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (if (looking-back ",")
...
(defun xingwenju/load-my-layout ()
  (interactive)
  (persp-load-state-from-file (concat persp-save-dir "zilong")))

(defun xingwenju/save-my-layout ()
  (interactive)
...
(defun xingwenju/indent-region(numSpaces)
  (progn
...
(defun xingwenju/tab-region (N)
  (interactive "p")
  (if (use-region-p)
      (xingwenju/indent-region 4)               ; region was selected, call indent-region
    (insert "    ")                   ; else insert four spaces as expected
    ))

(defun xingwenju/untab-region (N)
  (interactive "p")
  (xingwenju/indent-region -4))

(defun xingwenju/hack-tab-key ()
  (interactive)
  (local-set-key (kbd "<tab>") 'xingwenju/tab-region)
  (local-set-key (kbd "<S-tab>") 'xingwenju/untab-region)
  )

;; I'm don't like this settings too much.
;; (add-hook 'prog-mode-hook 'xingwenju/hack-tab-key)
...
(defun xingwenju/helm-hotspots ()
  "helm interface to my hotspots, which includes my locations,
org-files and bookmarks"
  (interactive)
  (helm :buffer "*helm: utities*"
        :sources `(,(xingwenju//hotspots-sources))))

(defun xingwenju//hotspots-sources ()
  ...
  `((name . "Mail and News")
    (candidates . (("Calendar" . (lambda ()  (browse-url "https://www.google.com/calendar/render")))
                   ...
                   ("Run current flie" . (lambda () (xingwenju/run-current-file)))
...
(defun xingwenju/browser-refresh--chrome-applescript ()
  (interactive)
  (do-applescript
   (format
    "
...
(defun xingwenju/open-file-with-projectile-or-counsel-git ()
  (interactive)
  (if (xingwenju/git-project-root)
      (counsel-git)
...
(defun xingwenju/growl-notification (title message &optional sticky)
...
(defun xingwenju/growl-timer (minutes message)
  ...
  (run-at-time (* (string-to-number minutes) 60)
               ...
               (lambda (minute message)
                 (xingwenju/growl-notification "Emacs Reminder" message t))
               minutes
               message))

(defun xingwenju/goto-match-paren (arg)
...
(defun xingwenju/hidden-dos-eol ()
  "Do not show ^M in files containing mixed UNIX and DOS line endings."
  (interactive)
  (setq buffer-display-table (make-display-table))
  (aset buffer-display-table ?\^M []))

(defun xingwenju/remove-dos-eol ()
  "Replace DOS eolns CR LF with Unix eolns CR"
  (interactive)
  (goto-char (point-min))
  (while (search-forward "\r" nil t) (replace-match "")))

(defun xingwenju/insert-chrome-current-tab-url()
  "Get the URL of the active tab of the first window only work in Mac"
  (interactive)
  (insert (xingwenju/retrieve-chrome-current-tab-url)))

(defun xingwenju/retrieve-chrome-current-tab-url()
...
(defun xingwenju/single-lines-only ()
...
(defun xingwenju/named-term (name)
  (interactive "sName: ")
  (ansi-term "/bin/zsh" name))


(defun xingwenju/ash-term-hooks ()
...
(defalias 'tt 'xingwenju/terminal)
...
(defun xingwenju/word-count-for-chinese ()
  "「較精確地」統計中/日/英文字數。
- 文章中的註解不算在字數內。
...
(defun xingwenju/evil-quick-replace (beg end )
  (interactive "r")
...
(defun xingwenju/git-project-root ()
...
(defun xingwenju/iterm-shell-command (command &optional prefix)
  "cd to `default-directory' then run COMMAND in iTerm.
With PREFIX, cd to project root."
  (interactive (list (read-shell-command
                      "iTerm Shell Command: ")
                     current-prefix-arg))
  (let* ((dir (if prefix (xingwenju/git-project-root)
                default-directory))
...
(defun xingwenju/my-mc-mark-next-like-this ()
  (interactive)
  (if (region-active-p)
...
(defun my-open-file-in-external-app (file)
  ...
  (interactive)
  (let ((default-directory (xingwenju/git-project-root))
        (file-path file))
    (if file-path
        (cond
...
(defun xingwenju/magit-visit-pull-request ()
...
(defun xingwenju/markdown-to-html ()
  (interactive)
...
(defun xingwenju/github-browse-commit ()
...
(defun xingwenju/search-in-minecraft ()
  (interactive)
  (helm-do-ag (expand-file-name "~/minecraft/")))


(defun xingwenju/show-current-buffer-major-mode ()
  (interactive)
  (describe-variable 'major-mode))

(defun xingwenju/counsel-imenu ()
  (interactive)
  (counsel-imenu)
  (evil-set-jump))

(defun xingwenju/delete-comment-lines()
  "Delete Procedure: Goto start point-kill ring-comment kill"
  (interactive)
  (goto-char (point-min))
  (let (kill-ring)
...
(defun xingwenju/delete-blank-lines()
  (interactive)
  (flush-lines "^$")
  )
